...
if(isset($_POST['sendButton'])){
    $bayarTunai = $_POST['bayarTunai'];
    $kembalian = $_POST['kembalian'];

    if($tanggal=="" || $bayarTunai=="" || $kembalian==""){
        $error = true;
    }else{
        $queryUpdateTblNota = mysqli_query($connect, "UPDATE tabel_nota SET TANGGAL='$tanggal', BAYAR_TUNAI='$bayarTunai', KEMBALIAN='$kembalian' WHERE ID_PELANGGAN='$idPelanggan'");
        if($queryUpdateTblNota){
            $queryUpdateTblPembelian = mysqli_query($connect, "UPDATE tabel_pembelian SET ID_NOTA = '$idNota' WHERE ID_PELANGGAN='$idPelanggan'");
            if($queryUpdateTblPembelian){
                $queryUpdateTblPelanggan = mysqli_query($connect, "UPDATE tabel_pelanggan SET ID_NOTA = '$idNota' WHERE ID_PELANGGAN='$idPelanggan'");
                if($queryUpdateTblPelanggan){
                    $_SESSION["idPelanggan"] = $idPelanggan;
                    header('Location:printNota.php');
                }
            }
            ...
            <div class="container-fluid d-flex flex-column p-0">
                ...
                <ul class="nav navbar-nav text-light" id="accordionSidebar">
                    <li class="nav-item" role="presentation">
                        <a class="nav-link" href="detailDataPembelian.php">
                            <i class="fas fa-times-circle border rounded-circle border-secondary" style="padding: 5px;"></i>
                            <span style="margin-left: 10px;font-weight: 200;">Batal</span></a>
                        </li>
                </ul>
                                    ...
                                    <form class="text-right user" method="POST">
                                        <div class="form-row text-right" style="margin-top: 20px;">
                                            <div class="col" style="max-width: 530px;">
                                                ...
                                                <div class="form-row">
                                                    <div class="col" style="width: 300px;padding-top: 5px;">
                                                        <h1 class="text-right" style="font-size: 16px;">Bayar :</h1>
                                                    </div>
                                                    <div class="col">
                                                        <input class="form-control form-control-user" type="number" id="bayarTunai" placeholder="0" name="bayarTunai" style="padding: 1px;border-radius: 0 0 10px 0;"></div>
                                                </div>
                                                <div class="form-row" style="margin-top: 5px;">
                                                    <div class="col" style="padding-top: 5px;">
                                                        <h1 class="text-right" style="font-size: 16px;">Kembalian :</h1>
                                                    </div>
                                                    <div class="col"><input class="form-control form-control-user" type="number" id="kembelian" placeholder="0" name="kembalian" style="padding: 1px;border-radius: 0 0 10px 0;"></div>
                                                </div>
                                                <div class="form-row" style="margin-top: 5px;">
                                                    <div class="col" style="padding-top: 5px;">
                                        ...
                                        <button name="sendButton" class="btn btn-primary" type="submit" style="border-radius: 1em;background-color: #ebac77;border-color: #ebac77;margin-top: 30px;margin-right: 20px;">
                                            <i class="fa fa-save" style="margin-right: 9px;"></i>Pesan
                                        </button>
                                        <hr>
                                    </form>
