...
$getDataMenu     = mysqli_query($connect, "SELECT * FROM tabel_menu WHERE ID_MENU = $idMenu");
while($showMenu = mysqli_fetch_array($getDataMenu)){
    $namaMenu    = $showMenu['NAMA_MENU'];
}

if(isset($_POST['sendJumlahItem'])){
    $jumlahItem        = $_POST["jumlahItem"];

    if($jumlahItem == ""){
        $error = true;
    }else{
        $queryBayarPerItem = mysqli_query($connect,"SELECT HARGA FROM tabel_menu WHERE ID_MENU=$idMenu");
        while($sumBayarPerItem = mysqli_fetch_array($queryBayarPerItem)){
            $bayarPerItem  = $sumBayarPerItem['HARGA'] * $jumlahItem;
        }
        $queryInsert  = mysqli_query($connect,"INSERT INTO tabel_pembelian (ID_PELANGGAN,ID_NOTA,ID_MENU,JUMLAH,BAYAR_ITEM) 
                                                VALUES ('$idPelanggan',NULL,'$idMenu','$jumlahItem','$bayarPerItem')");
        if($queryInsert){
                ...
                <ul class="nav navbar-nav text-light" id="accordionSidebar">
                    <li class="nav-item" role="presentation">
                        <a class="nav-link" href="detailDataPembelian.php">
                            ...
                            <span style="margin-left: 10px;font-weight: 200;">Batal</span>
                        </a>
                    </li>
                </ul>
                                ...
                                <div class="row">
                                    <div class="col">
                                        <form class="user" method="POST">
                                            <strong><?php echo $namaMenu ?></strong>
                                            <input class="form-control form-control-user" type="number" id="jumlahItem" aria-describedby="emailHelp" placeholder="0" name="jumlahItem" style="border-radius: 2em;max-width: 400px;margin-top: 5px;margin-bottom: 20px;">
                                            <input type="hidden" name="idPelanggan" value=<?php echo '$idPelanggan'; ?> >
                                            <div class="form-row">
                                                <div class="col text-right" style="margin-top: 10px;">
                                                    <button name="sendJumlahItem" class="btn btn-primary" type="submit" style="border-radius: 1em;background-color: #ebac77;border-color: #ebac77;width: 70px;">
